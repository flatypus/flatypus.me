...
class Person {
  ...
  Draw(positions, mousepos, screensize, mousedown) {
    ...
    if (this.x + 40 + (250 * this.showFrame / 12) > screensize[0])
      this.x = screensize[0] - 40 - (250 * this.showFrame / 12);
    if (this.y - 40 < 0) this.y = 40;
    if (this.y + 40 + (300 * this.showFrame / 12) > screensize[1])
      this.y = screensize[1] - 40 - (300 * this.showFrame / 12);

    // move people closer to their center of mass
    const [cx, cy] = [screensize[0] / 2, screensize[1] / 2];
    const dx = cx - this.x;
    const dy = cy - this.y;
    this.vx += dx * 0.0000025 * Math.abs(dx);
    this.vy += dy * 0.0000025 * Math.abs(dy);
